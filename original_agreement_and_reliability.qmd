...
agreeStudy <- function(ds){
  ds <- ds %>% mutate_all(.,toupper) %>% select(where(~ !any(is.na(.))))
...
agreeAI <- function(ds){
  ds <- ds %>%
    mutate(across(c(OC, AC, GG), toupper)) %>%
    select(OC, AC, GG) %>%
...
```{r}
lapply(alldata, agreeAI)
```


```{r}
testblah <- lapply(alldata, agreeStudy) %>% melt() %>% select(1)
...
kappaAI <- function(ds){
  ...
  kappam.fleiss(ds, detail = TRUE) %>% return()

}
```

```{r}
lapply(alldata, kappaAI)
```
